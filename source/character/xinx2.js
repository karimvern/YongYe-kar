...
export let info = {
    ...
    characterSort: {
        xinx2: {
            'liechexinghe': ['xinx_liuying', 'xinx_ren', 'xinx_qiong', 'xinx_kafuka', 'xinx_sanyueqi', 'xinx_yinglang', 'xinx_danheng', 'xinx_xing'],
    ...
    character: {
        ...
        //xinx_kafuka: ["female", "xinx", 3, [], ['epic']],
    ...
    //武将替换
    characterReplace: {
        xinx_liuying: ['xinx_liuying', 'xinxmeng_liuying'],
    ...
    characterSubstitute: {//皮肤切换
        ...
        xinx_feixiao: [
            ...
            ['xinx_feixiao_shadow4', [((lib.device || lib.node) ? 'ext:' : 'db:extension-') + '永夜之境/image/xinx_feixiao_shadow4.png']],
            ['xinx_feixiao_shadow5', [((lib.device || lib.node) ? 'ext:' : 'db:extension-') + '永夜之境/image/xinx_feixiao_shadow5.png']],
            ['xinx_feixiao_shadow6', [((lib.device || lib.node) ? 'ext:' : 'db:extension-') + '永夜之境/image/xinx_feixiao_shadow6.png']],
        ],
        xinx_huangquan: [
            ['xinx_huangquan_shadow', [((lib.device || lib.node) ? 'ext:' : 'db:extension-') + '永夜之境/image/xinx_huangquan_shadow.png']],
        ],
        xinx_baie: [
            ['xinx_baie_shadow', [((lib.device || lib.node) ? 'ext:' : 'db:extension-') + '永夜之境/image/xinx_baie_shadow.png']],
        ],
        // xinx_sanyueqi: [
        //     ['xinx_sanyueqi_shadow', [((lib.device || lib.node) ? 'ext:' : 'db:extension-') + '永夜之境/image/xinx_sanyueqi_shadow.png']],
        // ],
    ...
    skill: {
                ...
                if (result.bool) {
                    ...
                    if (!player.storage.xinxmeng) {
                        ...
                        target.when({ global: 'useCardAfter' })
                            ...
                            .then(() => player.removeSkill('xinxleizhi_effect'));
                    } else {
                        await target.addTempSkill('xinxleizhi_effect');
                        // target.when({ global: 'phaseAfter' })
                        //     .filter(evt => evt === trigger.getParent())
                        //     .then(() => player.removeSkill('xinxleizhi_effect'));
                    }
                }
        ...
        xinxzhuhuo: {
            ...
            async content(event, trigger, player) {
                ...
                if (count > hp) {
                    let num = count - hp;
                    let historyLen = target.getHistory('lose').length;
                    await target.chooseToDiscard(num, 'he', true);
                    let newHistory = target.getHistory('lose');
                    if (newHistory.length > historyLen) {
                        // 获取最后一次失去的牌记录
                        let lastLose = newHistory[newHistory.length - 1];
                        if (lastLose.cards && lastLose.cards.length) {
                            cards_processed = lastLose.cards;
                            changed = true;
                        }
                    }
                } else if (count < hp) {
                    let num = hp - count;
                    // 记录目标当前的“获得牌”历史事件数量
                    let historyLen = target.getHistory('gain').length;
                    await target.draw(num);
                    let newHistory = target.getHistory('gain');
                    if (newHistory.length > historyLen) {
                        // 取出最后一次（也就是刚刚那次）获得的牌
                        let lastGain = newHistory[newHistory.length - 1];
                        if (lastGain.cards && lastGain.cards.length) {
                            cards_processed = lastGain.cards;
                            changed = true;
                        }
                    }
                }
                if (!changed) {
                    //player.logSkill("xinxzhuhuo", [target], null, null, [get.rand(1, 2)]);
        ...
        xinxyongjie: {
            ...
            async content(event, trigger, player) {
                ...
                if (cardsToPile.length > 0) {
                    for (let i = 0; i < cardsToPile.length; i++) {
                        const c = cardsToPile[i];
                        c.fix(); // 修正卡牌信息

                        const pileLen = ui.cardPile.childElementCount;
                        const randomPos = Math.floor(Math.random() * (pileLen + 1));

                        // 插入到 UI 界面
                        if (ui.cardPile.children[randomPos]) {
                            ui.cardPile.insertBefore(c, ui.cardPile.children[randomPos]);
                        } else {
                ...
                aocai: {
                    ...
                    onChooseToUse(event) {
                        ...
                        if (ui.cardPile.hasChildNodes !== false) {
                            ...
                            const hasBottom = player.storage.xinxzhuoxiao; // 【修改】开关
                            // 【修改】默认生成顶部
                            const topCards = pile.slice(0, num);
                            // 【修改】卓效生成底部（去重）
                            const bottomCards = hasBottom ? pile.slice(-num).filter(c => !topCards.includes(c)) : [];
                ...
                if (totalCards > 0) {
                    ...
                    const result1 = await player.chooseButton(dialog)
                        .set('ai', (button) => {
                            ...
                            //     return -100;
                            // }
                            const targetSet = new Set(['xinxqinchen', 'guohe', 'sha', 'mb_qingnangshu', 'xinxnilin', 'xinxjinshouzhi']);
            ...
            subSkill: {
                ...
                effect: {
                    ...
                    filter(event, player) {
                        ...
                        return event.targets?.some(i => i !== player && i.isAlive());
                    },
                    forced: true,
                    popup: false,
                    logTarget: "target",
        ...
        xinxbuli: {
            ...
            async content(event, trigger, player) {
                ...
                while (judgeCount < player.storage.xinxbuli) {
                    ...
                    if (card && endHistoryLength > startHistoryLength) {
                        ...
                        //player.storage.xinxbuli_gain_cards.push(card);
            ...
            subSkill: {
                gain: {
                    ...
                    async content(event, trigger, player) {
                        //const cardsToGain = [];
                        // for (const card of player.storage.xinxbuli_gain_cards) {
        ...
        xinxguiren: {
            ...
            ai: {
                order(item, player, target) {
                    return Math.max(get.order({ name: "sha" })) + 0.2;
                },
                result: {
                    player: 1,
                },
        ...
        xinxzhuiyuan: {
            ...
            async content(event, trigger, player) {
                ...
                if (performYinEffect) {
                    ...
                    player.addSkill('xinxzhuiyuan_add');
                    const type = get.type(cards[0]);
                    if (type === 'basic' || type === 'trick') {
                        await player.addToExpansion(cards, "gain2").set("gaintag", ["xinxzhuiyuan_add"]);
                    }
                }
                if (shouldChange) {
            ...
            subSkill: {
                add: {
                    ...
                    intro: {
                        mark(dialog, storage, player) {
                            const cards = player.getExpansions("xinxzhuiyuan_add");
                            if (player.isUnderControl(true)) {
                                dialog.addAuto(cards);
                            } else {
        ...
        xinxliuduan: {
            ...
            async content(event, trigger, player) {
                ...
                if (!isYin) {
                    // --- 当前是【阳】 ---
                    const expCards = player.getExpansions("xinxzhuiyuan_add");
