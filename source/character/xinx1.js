...
import { xinx1Character as character } from "./character.js";
...
export let info = {
    ...
    translate: {
        ...
        xinxbeifa: '北伐',
        xinxbeifa_info: "锁定技。①当你指定或成为牌的唯一目标时，若此牌不为转化牌且对应的实体牌牌数为1且不为装备牌且此时在一名角色的出牌阶段，则你将此牌置于目标角色的武将牌上，称为“伐”。②一名角色的回合开始时，你可以依次使用其武将牌上的“伐”。",
        ...
        xinxjishi: '犄势',
        xinxjishi_info: `锁定技，当一名角色不因本技能使用不为其手牌区的${get.poptip('xinx_jishipai')}时，你视为使用一张同名牌（无距离限制）。`,
        ...
        xinxweixi: '未息',
        xinxweixi_info: "①回合开始前，你获得一张【影】，然后将任意张牌置于牌堆底并摸等量的牌。②每回合限一次。你可以展示手牌中的【影】，视为使用一张基本牌。",
    ...
    characterReplace: {
        ...
        xinx_zhonghui: ['xinx_zhonghui', 'xinxying_zhonghui', 'xinxnew_zhonghui'],
    ...
    skill: {
        ...
        xinxsaran: {
            ...
            async content(event, trigger, player) {
                const target = event.targets[0];
                const dialog = ui.create.dialog('hidden', `###${get.prompt(event.name)}###<div class="text center">选择你与${get.translation(target)}各至多3张牌</div>`);
                ...
                if (ecards.length) {
                    dialog.addText(`<div class="text center">${get.translation(target)}的装备</div>`);
                    dialog.add(ecards);
                }


                const result2 = await player.chooseButton(dialog, [1, 6])
                    .set('filterButton', button => {
                        const card = button.link;
        ...
        xinxshiren: {
            ...
            async content(event, trigger, player) {
                ...
                for (const name of list) {
                    skillMap[name] = (lib.character[name][3] || []).filter(filter);
                }

                if (!Object.keys(skillMap).length) return;

                const result = await player
                    .chooseButton(
                        [
                            [["识人：请选择获得其中一个技能"], "addNewRow"],
                            [
                                dialog => {
                                    const { list, skillMap } = get.event();
                                    // 算出来需要多少列，最多等于抽取武将的数量（5列）
                                    const column = list.length;
                                    if (column > 4) {
                                        dialog.css({
                                            width: "100%",
                                            left: 0,
                                    ...
                                    for (const i of list) {
                                        ...
                                        if (skills.length) {
                                            ...
                                            // 只有技能按钮被加到 dialog.buttons 里，武将头像是点不了的
        ...
        xinxshisheng: {
            ...
            locked: true,
            filter(event, player) {
                let storage = player.getStorage("xinxtianzhi");
                if (!storage.length) return false;
                let storage2 = player.getStorage("xinxtianzhi_used");
                return storage.some(name => {
                    return !storage2.includes(name) && event.filterCard(get.autoViewAs({ name }, "unsure"), player, event);
                });
            },
            hiddenCard(player, name) {
                let list = player.getStorage("xinxtianzhi");
            ...
            chooseButton: {
                dialog(event, player) {
                    let list = [];
                    for (let name of player.storage.xinxtianzhi) {
                        if (name == "sha") {
                            if (event.filterCard(get.autoViewAs({ name }, "unsure"), player, event)) list.push(["基本", "", "sha"]);
                ...
                filter(button, player) {
                    let card = { name: button.link[2], storage: { xinxtianzhi: true } };
        ...
        xinxqianlong: {
            ...
            filter(event, player) {
                /* if (!["useCard"].includes(event.name)) return event.name !== "phase" || game.phaseNumber === 0;
                if (event.name === "useCard") {
        ...
        xinxxzhaohan: {
            ...
            clickableFilter(player) {
                //return player.isPhaseUsing();
                return player.hasSkill("xinxxzhaohan");
            },
            clickable(player) {
        ...
        xinxduanzhi: {
            ...
            filter(event, player) {
                if (event.player === player) {
                    ...
                    }) && player.hasMark("xinxjinshu");
                }
                return event.player.hasHistory("useCard", evt => {
                    return (evt.cards || []).some(card => get.position(card, true) == "d");
                }) && !player.hasSkill("xinxduanzhi_used");
            },
            async cost(event, trigger, player) {
                event.result = await player
            ...
            async content(event, trigger, player) {
                if (trigger.player !== player) {
                    player.addTempSkill("xinxduanzhi_used", 'roundEnd');
                } else {
                    player.clearMark('xinxjinshu', false);
                }
            ...
            async content(event, trigger, player) {
                ...
                const result = await player
                    .chooseTarget(get.prompt2("xinxyunjue"), "选择一名角色的一张牌令" + get.translation(trigger.player) + "获得", function (card, player, target) {
                        if (player.storage.xinxyunjue_count < 3) {
                            return target != trigger.player && target.countCards('he')
                        }
                        return target.countCards('he');
                    })
                    .set("ai", target => {
                ...
                if (result.bool) {
                    ...
                    let { cards } = await player
                        .choosePlayerCard("韫决：选择" + get.translation(target) + "的一张牌", target, "he", true)
                        .set("ai", function (button) {
                        ...
                        for (const markedTarget of markedTargets) {
                            ...
                            if (discard) {
                                ...
                                for (const markedTarget of markedTargets) {
                                    ...
                                    if (player.canUse({ name, nature, isCard: true }, markedTarget, false)) {
                                        await player.useCard({ name, nature, isCard: true }, markedTarget, false);
                                    }
                                }
                            }
                        }
